...
graduates.forEach(function (universityname) {
  universities.push(universityname.university)
})
...
//The above array from Request 1 has university names stored in it. I will run includes()
...
const lowerCase = [];
zooAnimals.map(
  small => lowerCase.push(small.animal_name.toLowerCase())
)
...
lowerPopulation.push(zooAnimals.filter(size => size.population < 5))
...
populationTotal += zooAnimals.reduce((acc, count) => {
...
Converted Array Methods to arrow functions
*/
